...
<body>
    ...
    <div class="container">
        <div class="text">Liste des Enseignants</div>
    </div>
            ...
            <tbody>
                @foreach($enseignants as $ens)
                    <tr>
                        ...
                        <td>
                            <!-- Edit icon -->
                            <a href="" class="action-icon" data-bs-toggle="modal" data-id="{{ $ens->id }}" data-name="{{ $ens->name }}" data-matiere="{{ $ens->matiere }}" data-email="{{ $ens->email }}" data-password="{{ $ens->password }}" data-bs-target="#EnseignantEditModal"><i class="bx bx-edit"></i></a>
                            <!-- Delete icon -->
                            <a href="" class="delete-icon" data-id="{{ $ens->id }}" ><i class='bx bxs-x-circle' ></i></a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        ...
        <!-- Modal -->
        <div class="modal fade" id="EnseignantModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <!--             <label for="matiere">matiere:</label>
        ...
        <!-- Modal -->
        <div class="modal fade" id="EnseignantEditModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="exampleModalLabel">Modifier un enseignant</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <label for="editMatiere">Matiere:</label>
                        <input type="text" class="form-control" id="editMatiere">
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                console.log('Form Data:', { name, matiere, email, password });

                // Perform AJAX request to server
                $.ajax({
                    url: "{{ route('enseignant.store') }}", // Update with your route
                    type: 'POST',
                    dataType: 'json',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    data: { name, matiere, email, password },
                ...
                $('#saveBTNEdit').on('click', function() {
                    ...
                    var matiere = $('#editMatiere').val();
                    var email = $('#editEmail').val();
                    var password = $('#editMdp').val();

                    $.ajax({
                        url: "{{ route('enseignant.update', '') }}" + '/' + id,
                        type: "PATCH",
                        dataType: "json",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        data: { name: name, matiere: matiere, email: email, password: password },
                        success: function(response) {
                    ...
                    $('#EnseignantEditModal').modal('hide');
                });
                $('.delete-icon').on('click', function(event) {
                    ...
                    if (confirm('Voulez vous supprimer?')) {
                        $.ajax({
                            url: "{{ route('enseignant.destroy', '') }}" + '/' + id,
                            type: "DELETE",
                            dataType: "json",
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
